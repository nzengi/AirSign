...
///
/// **Memory hygiene:** `secret_package_json` is zeroized on `Drop`.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Drop for DkgRound1Output {
    fn drop(&mut self) {
        self.secret_package_json.zeroize();
    }
}
...
///
/// **Memory hygiene:** `key_package_json` is zeroized on `Drop`.
#[derive(Debug, Clone, Serialize, Deserialize)]
