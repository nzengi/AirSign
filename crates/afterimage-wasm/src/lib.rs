...
// ─── Password strength assessment (Q3 hardening) ─────────────────────────────

/// Estimate the entropy of a session password, in bits.
///
/// See `afterimage_core::password` for methodology and recommended thresholds.
/// Use this in the UI to surface a strength indicator before the user commits
/// to an Argon2id round.
#[wasm_bindgen]
pub fn password_entropy_bits(password: &str) -> f64 {
    afterimage_core::password::entropy_bits(password)
}

/// Returns a category label for the password: `"weak"`, `"medium"`,
/// `"strong"`, or `"paranoid"`. Use it to drive the UI badge color.
#[wasm_bindgen]
pub fn password_strength_label(password: &str) -> String {
    let bits = afterimage_core::password::entropy_bits(password);
    afterimage_core::password::PasswordStrength::from_bits(bits)
        .label()
        .to_owned()
}

/// Returns `true` iff the password meets AirSign's mainnet recommendation
/// (≥ 60 bits of estimated entropy).  Use it to enable/disable the
/// "broadcast to mainnet" button.
#[wasm_bindgen]
pub fn password_is_mainnet_ready(password: &str) -> bool {
    let bits = afterimage_core::password::entropy_bits(password);
    afterimage_core::password::PasswordStrength::from_bits(bits)
        .is_mainnet_ready()
}
...
impl WasmKeypair {
    ...
    pub fn generate() -> Result<WasmKeypair, JsValue> {
        ...
        // Zero the temporary seed buffer — `SigningKey::from_bytes` already
        // copied the bytes into its internal state, and ed25519-dalek's
        // `zeroize` feature wipes that copy on `Drop`.
        seed.zeroize();
        Ok(kp)
    }
    ...
    ///
    /// The caller's `seed` slice is borrowed and untouched. The local 32-byte
    /// copy used to build the SigningKey is zeroized before this function
    /// returns.
    ///
    /// # Errors
...
/// ```
/// **Memory hygiene:** the `key_package_json` field holds a serialised secret
/// signing share. The struct's `Drop` impl zeroizes the heap allocation so the
/// share can't be recovered by a process-memory dump after the participant
/// finishes signing.
#[wasm_bindgen]
...
impl Drop for WasmFrostParticipant {
    fn drop(&mut self) {
        use zeroize::Zeroize;
        self.key_package_json.zeroize();
    }
}
