...
pub enum ProtocolError {
    ...
    /// METADATA `k` (block count) exceeds the resource-exhaustion limit.
    ///
    /// Prevents a malicious frame from triggering a multi-gigabyte allocation
    /// in the receiver. The cap (`MAX_BLOCK_COUNT`) is intentionally generous
    /// so legitimate large payloads still pass.
    #[error("METADATA k={got} exceeds limit {max}; possible resource-exhaustion attempt")]
    BlockCountTooLarge {
        /// Block count claimed by the frame.
        got: u32,
        /// Maximum block count this build will accept.
        max: u32,
    },

    /// METADATA `original_len` exceeds the resource-exhaustion limit.
    #[error("METADATA original_len={got} exceeds limit {max}; possible resource-exhaustion attempt")]
    OriginalLenTooLarge {
        /// Plaintext length claimed by the frame.
        got: u32,
        /// Maximum length this build will accept.
        max: u32,
    },
}