...
// ─── Resource-exhaustion bounds (Q7 / Q19 hardening) ─────────────────────────
//
// Without these, a hostile METADATA frame with `k = u32::MAX` would trigger a
// ~1 TB allocation in the LT decoder (`vec![[0u8; 256]; k]`), instantly
// killing the WASM runtime or the host process. The legitimate AirSign use
// case is signing 200-byte to ~64 KB transactions, so the cap is set
// generously above that.

/// Maximum block count we accept from a METADATA frame.
///
/// 32 768 blocks × 256-byte BLOCK_SIZE = 8 MiB plaintext budget — far above
/// any legitimate Solana transaction or Squads multi-instruction payload,
/// while keeping the decoder's allocations bounded to a few MiB.
pub const MAX_BLOCK_COUNT: u32 = 32_768;

/// Maximum decompressed plaintext length we accept from a METADATA frame.
///
/// 16 MiB — gives breathing room for arbitrary application payloads while
/// preventing the receiver from being told to allocate gigabytes for
/// reassembly buffers it can't possibly need.
pub const MAX_ORIGINAL_LEN: u32 = 16 * 1024 * 1024;
...
impl MetadataFrame {
    ...
    pub fn from_bytes(frame: &[u8]) -> Result<Self, ProtocolError> {
        ...
        // ── Q7/Q19 hardening — reject frames that would trigger a
        //    multi-gigabyte allocation in the decoder.  These bounds run
        //    BEFORE any `vec!` allocation is attempted on `k` or `original_len`.
        if k > MAX_BLOCK_COUNT {
            return Err(ProtocolError::BlockCountTooLarge {
                got: k,
                max: MAX_BLOCK_COUNT,
            });
        }
        if original_len > MAX_ORIGINAL_LEN {
            return Err(ProtocolError::OriginalLenTooLarge {
                got: original_len,
